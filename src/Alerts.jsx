...
import { 
  collection, query, onSnapshot, updateDoc, doc, getDoc 
} from 'firebase/firestore';
import { 
  Bell, CheckCircle, Clock, CalendarDays, ExternalLink, ChevronLeft,
  LogOut, MoreVertical, FileText, CheckSquare, TrendingUp, AlertCircle,
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 mt-0.5 truncate flex items-center gap-1">
            {item.source === 'task' ? "Task" : item.source.charAt(0).toUpperCase() + item.source.slice(1)} • {item.calendarName || getSourceLabel(item.source)}
...
const AlertsApp = ({ user, cryptoKey, onExit, onLaunch }) => { // UPDATED: Added onLaunch prop
  ...
  useEffect(() => {
    ...
    const unsubs = fetchers.map(({ col, type, setter }) => {
        ...
        return onSnapshot(q, async (snap) => {
            ...
            if(col === 'counters') setLoading(false);
        });
    });
              ...
              try {
                  ...
                  if (response.result.items) {
                      ...
                      const items = response.result.items.map(ev => ({
                          id: ev.id, source: 'calendar', title: ev.summary || "Busy", date: new Date(ev.start.dateTime || ev.start.date), link: ev.htmlLink, calendarName: calName, category: categorizeItem(new Date(ev.start.dateTime || ev.start.date))
                      }));
                  ...
                  if (e.status === 401) {
                      setGcalSignedIn(false); 
                      setGcalError("Session expired. Please reconnect.");
                  }
              }
          }
              ...
              if (userDoc.exists() && userDoc.data().gcal_token) {
                  ...
                  if (token && token.access_token) {
                      ...
                      if (timeLeft > 60000) { 
                          ...
                          setGcalError("Session expired. Please connect again.");
  ...
  useEffect(() => {
      if (gcalSignedIn && gapiInited) fetchGcalEvents();
  ...
  const handleSnooze = async (item) => {
    try {
        ...
        const newData = { ...item.original, dueDate: newDate.toISOString() };
  ...
  // UPDATED: Use onLaunch prop for navigation
  const handleNavigate = (item) => {
      if (item.source === 'calendar') {
          window.open(item.link, '_blank');
      } else {
          // Map source type to App ID
          const appMap = { task: 'tasks', note: 'notes', checklist: 'checklist', counter: 'counter' };
          const targetApp = appMap[item.source];
          
          if (targetApp && onLaunch) {
              onLaunch(targetApp); // Switches the view in App.jsx
          } else {
              console.warn("Navigation failed: onLaunch missing or invalid source");
          }
      }
  };
  ...
  const { grouped, focusItem, totalCounts } = useMemo(() => {
    const combined = [...tasks, ...notes, ...checklists, ...counters, ...calendarEvents].sort((a, b) => a.date - b.date);
  ...
  }, [tasks, notes, checklists, counters, calendarEvents]);

  // --- Auto-Switch ---
  useEffect(() => {
      if (!loading && !autoSwitched && gcalSignedIn) {
          if (totalCounts.today === 0 && totalCounts.tomorrow === 0 && totalCounts.this_week > 0) {
              setActiveTab('this_week');
              setAutoSwitched(true);
          }
      }
  }, [loading, totalCounts, autoSwitched, gcalSignedIn]);
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
        ...
        <div className="max-w-3xl mx-auto px-4 pt-4 flex flex-col gap-4">
          ...
          {focusItem ? (
             <div className={`rounded-2xl p-4 text-white shadow-md transform transition-all active:scale-[0.99] cursor-pointer ${getSourceColor(focusItem.source).replace('text-', 'from-').replace('50', '500').replace('500', '600')} bg-gradient-to-r to-gray-400`} onClick={() => handleNavigate(focusItem)}>
                ...
                <div className="mt-2 flex items-center gap-2 text-xs font-medium text-white/90"><span className="bg-black/20 px-2 py-0.5 rounded">{getRelativeTime(focusItem.date)}</span><span>• {focusItem.calendarName || getSourceLabel(focusItem.source)}</span></div>
             </div>
          ) : (
      ...
      <main className="flex-1 overflow-y-auto scroll-smooth p-4 bg-gray-50">
        <div className="max-w-3xl mx-auto pb-20">
          {loading ? <div className="flex justify-center py-20"><LoadingSpinner /></div> : grouped[activeTab].length === 0 ? (
            <div className="text-center py-20 text-gray-400 flex flex-col items-center gap-3">
                ...
                {activeTab === 'today' && <button onClick={fetchGcalEvents} className="text-xs text-blue-500 flex items-center gap-1 hover:underline"><RefreshCw size={12} /> Refresh Calendar</button>}
            </div>
          ) : (
