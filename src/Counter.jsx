...
export default function CounterApp({ user, cryptoKey, onExit }) {
        ...
        if (event.state?.appId === 'counter') {
            if (event.state.view === 'detail' && event.state.counterId) {
                ...
                if (counter) {
                    ...
                    setView('detail');
                    // Optional: Restore tab if you wanted to save that in history too
                } else {
                    // Counter not found (deleted?), go to list
                    setView('list');
  ...
  // Handle opening a counter (Push State)
  const handleOpenCounter = (counter) => {
    window.history.pushState(
        { appId: 'counter', view: 'detail', counterId: counter.id },
        '',
        '#counter'
    );
    setSelectedCounter(counter);
  ...
  // Handle Back Button (UI)
  const handleBack = () => {
    if (view === 'detail') {
        window.history.back(); // Triggers popstate -> goes to list
    } else {
        onExit(); // Triggers history.back() in parent -> exits app
    }
  };
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      const data = await Promise.all(snapshot.docs.map(async doc => {
        ...
        return { id: doc.id, ...raw, ...decrypted };
      }));
      setCounters(data);
      setLoading(false);
  ...
  const handleCreateCounter = async (e) => {
    ...
    // NEW: Capture checkbox values
    const useTags = form.useTags.checked;
    const useNotes = form.useNotes.checked;
    
    if (!title) return;

    const encryptedData = await encryptData({ title }, cryptoKey);
  ...
  const handleSaveEntry = async (e) => {
    ...
    // Check if fields exist before trying to read values
    const sensitiveData = {};
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 text-gray-900 font-sans overflow-hidden">
      ...
      <header className="flex-none bg-[#4285f4] text-white p-4 shadow-md z-10">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            {/* UPDATED: Uses handleBack for proper history support */}
            <button onClick={handleBack} className="p-1 hover:bg-white/20 rounded-full transition-colors"><ChevronLeft size={24} /></button>
            <h1 className="text-xl font-bold flex items-center gap-2">{view === 'detail' ? selectedCounter.title : 'My Counters'}</h1>
          </div>
        </div>
      </header>
        ...
        <div className="max-w-3xl mx-auto p-4">
          {loading ? <LoadingSpinner /> : view === 'list' ? (
            <div className="grid gap-4">
              {counters.length === 0 && <div className="text-center py-20 text-gray-400"><p className="mb-4">No counters yet</p><Button onClick={() => setIsCreateModalOpen(true)}>Create your first</Button></div>}
              {counters.map(counter => (
                <div key={counter.id} onClick={() => handleOpenCounter(counter)} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between active:scale-[0.99] transition-transform cursor-pointer">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mt-1">
                        ...
                        {/* Optional badges to show features */}
                        {counter.useTags && <span className="text-[10px] bg-gray-100 text-gray-500 px-1.5 rounded flex items-center gap-0.5"><Tag size={10} /> Tags</span>}
                        {counter.useNotes && <span className="text-[10px] bg-gray-100 text-gray-500 px-1.5 rounded flex items-center gap-0.5"><StickyNote size={10} /> Notes</span>}
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between mb-1">
                 <h2 className="text-2xl font-bold text-gray-800">{selectedCounter.title}</h2>
      ...
      {/* (Kept exactly as previous version to save space - they are UI only) */}
      <div className="fixed bottom-6 right-4 z-20 md:hidden">
        {view === 'list' ? (
          <button onClick={() => setIsCreateModalOpen(true)} className="h-14 w-14 rounded-full bg-[#4285f4] text-white shadow-lg flex items-center justify-center active:scale-90 transition-transform"><Plus size={28} /></button>
        ) : (
      ...
      <div className="hidden md:block fixed bottom-6 right-[calc(50%-20rem)] z-20">
         <Button onClick={() => view === 'list' ? setIsCreateModalOpen(true) : (() => { setEditingEntry(null); setIsAddEntryModalOpen(true); })()} className="rounded-full shadow-xl py-4 px-6 text-lg flex items-center gap-2" disabled={view === 'detail' && activeTab === 'stats'}>
      ...
      <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} title="New Counter">
        <form onSubmit={handleCreateCounter} className="flex flex-col gap-4">
          <Input name="title" label="Counter Name" placeholder="e.g. Workouts, Water Intake" autoFocus required />
          <div className="flex flex-col gap-2 mb-2">
             <label className="text-sm font-medium text-gray-700">Tracking Mode</label>
             <div className="grid grid-cols-2 gap-3">
               <label className="cursor-pointer"><input type="radio" name="mode" value="date" defaultChecked className="peer sr-only" /><div className="p-3 border rounded-lg peer-checked:border-[#4285f4] peer-checked:bg-blue-50 peer-checked:text-[#4285f4] flex flex-col items-center gap-2 text-gray-500 hover:bg-gray-50 transition-all"><Calendar size={24} /><span className="text-xs font-semibold">Date Point</span></div></label>
               <label className="cursor-pointer"><input type="radio" name="mode" value="range" className="peer sr-only" /><div className="p-3 border rounded-lg peer-checked:border-[#4285f4] peer-checked:bg-blue-50 peer-checked:text-[#4285f4] flex flex-col items-center gap-2 text-gray-500 hover:bg-gray-50 transition-all"><Clock size={24} /><span className="text-xs font-semibold">Duration</span></div></label>
             </div>
          </div>
          
          <div className="flex gap-4">
            <label className="flex items-center gap-2 text-sm text-gray-700 font-medium cursor-pointer">
                <input type="checkbox" name="useTags" defaultChecked className="w-4 h-4 rounded text-[#4285f4] focus:ring-[#4285f4] border-gray-300" />
                Enable Tags
            </label>
            <label className="flex items-center gap-2 text-sm text-gray-700 font-medium cursor-pointer">
                <input type="checkbox" name="useNotes" defaultChecked className="w-4 h-4 rounded text-[#4285f4] focus:ring-[#4285f4] border-gray-300" />
                Enable Notes
            </label>
          </div>
          ...
          <div className="flex flex-col gap-2 mb-4">
             ...
             <div className="relative">
               <select name="groupBy" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#4285f4] focus:border-[#4285f4] outline-none bg-white appearance-none">
          ...
          <Button type="submit" className="w-full py-3">Create Counter</Button>
        </form>
      </Modal>
