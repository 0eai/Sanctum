...
import {
    collection, query, onSnapshot, doc, getDoc, setDoc, updateDoc
...
// --- ALERT ACTIONS (moved from Alerts.jsx to keep crypto out of components) ---

export const completeAlertItem = async (userId, cryptoKey, item) => {
    if (item.source === 'task') {
        const encrypted = await encryptData({ ...item.original, completed: true }, cryptoKey);
        await updateDoc(doc(db, 'artifacts', appId, 'users', userId, 'tasks', item.id), encrypted);
    } else if (item.source === 'reminder') {
        const encrypted = await encryptData({ ...item.original, isActive: false }, cryptoKey);
        await updateDoc(doc(db, 'artifacts', appId, 'users', userId, 'reminders', item.id), encrypted);
    }
};

export const snoozeAlertItem = async (userId, cryptoKey, item) => {
    ...
    await setDoc(doc(db, 'artifacts', appId, 'users', userId, collectionName, item.id), encrypted, { merge: true });
};