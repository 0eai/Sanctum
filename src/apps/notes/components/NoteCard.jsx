...
import {
    Folder, FileText, Globe, Star, Clock, RotateCcw, Paperclip, Share2, ArrowRightLeft, Edit2, Trash2, RefreshCw, Users
} from 'lucide-react';
import { formatBadgeDate } from '../../../lib/dateUtils';
import { usePermissions } from '../../../hooks/usePermissions';

const NoteCard = ({ item, viewMode, onOpen, onFolderOpen, onPin, onMove, onEditFolder, onDelete, onShare, onReschedule, onCollaborate, folderCounts, readOnly }) => {
        ...
        >
            ...
            {!readOnly && (
                <div className={`absolute top-2 right-2 flex flex-col gap-1 transition-opacity opacity-100 md:opacity-0 md:group-hover:opacity-100 ${viewMode === 'list' ? 'relative top-auto right-auto flex-row' : ''}`}>
                    {item.type === 'note' ? (
                        <>
                            {onCollaborate && <button onClick={(e) => onCollaborate(e, item)} className={`p-1.5 bg-white shadow-sm border border-gray-100 rounded-full ${item.memberUids?.length > 0 ? 'text-blue-500' : 'text-gray-400'} hover:text-blue-600 active:scale-95`} title="Collaborate"><Users size={14} /></button>}
                            {canShare && onShare && <button onClick={(e) => onShare(e, item)} className={`p-1.5 bg-white shadow-sm border border-gray-100 rounded-full ${item.sharedId ? 'text-green-500' : 'text-gray-400'} hover:text-blue-500 active:scale-95`} title="Public Link"><Share2 size={14} /></button>}
