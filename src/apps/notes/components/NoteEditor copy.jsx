...
import FileViewer from '../../../components/ui/FileViewer'; // Import the new component
...
const NoteEditor = ({ note, onSave, onBack, onPin, onShare, saveStatus }) => {
  ...
  const [viewingAttachment, setViewingAttachment] = useState(null); // State for viewer
                ...
                <div className="flex flex-wrap gap-2 items-center text-xs">
                    ...
                    {data.dueDate ? (
                        <div className="bg-blue-50 text-blue-600 px-2.5 py-1.5 rounded-full flex items-center gap-1.5 font-medium group relative overflow-hidden">
                            <Clock size={12} />
                            <span>{formatAlertDate(data.dueDate)}</span>
                            <input 
                                type="datetime-local" 
                                className="absolute inset-0 opacity-0 cursor-pointer text-[16px]" 
                                value={data.dueDate}
                                onChange={(e) => setData(s => ({...s, dueDate: e.target.value}))}
                            />
                            <button onClick={() => setData(s => ({...s, dueDate: null, repeat: 'none'}))} className="hover:text-red-500 z-10"><X size={12} /></button>
                        </div>
                    ) : (
                        <label className="flex items-center gap-1 text-gray-400 px-2.5 py-1.5 rounded-full border border-dashed border-gray-300 hover:border-[#4285f4] hover:text-[#4285f4] cursor-pointer transition-colors relative">
                    ...
                    {data.dueDate && (
                        <div className={`px-2.5 py-1.5 rounded-full flex items-center gap-1.5 font-medium relative ${data.repeat !== 'none' ? 'bg-purple-50 text-purple-600' : 'text-gray-400 border border-dashed border-gray-300 hover:border-purple-400 hover:text-purple-500'}`}>
                            <RotateCcw size={12} />
                            <span>{data.repeat === 'none' ? 'Repeat' : data.repeat}</span>
                            <select 
                                value={data.repeat} 
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-thin">
                        {data.attachments.map((att, i) => (
                            ...
                            >
                                ...
                                {/* Remove Button: 
                                    - Mobile (Default): Opacity 100, visible red background
                                    - Desktop (md): Opacity 0, fades in on hover 
                                */}
                                <button 
