...
export const saveEntry = async (userId, counterId, cryptoKey, entryData, counterMeta) => {
    ...
    if (id) {
        ...
        await setDoc(doc(db, 'artifacts', appId, 'users', userId, 'counters', counterId), counterUpdates, { merge: true });
    }
};
...
export const startTimer = async (userId, counterId) => {
    ...
    await setDoc(doc(db, 'artifacts', appId, 'users', userId, 'counters', counterId), { count: increment(1) }, { merge: true });
};
...
export const stopTimer = async (userId, counterId, entryId, counterMeta, cryptoKey) => {
    ...
    if (counterMeta.repeat && counterMeta.repeat !== 'none' && counterMeta.dueDate) {
        ...
        await setDoc(doc(db, 'artifacts', appId, 'users', userId, 'counters', counterId), encryptedMeta, { merge: true });
    }
};
