...
export const rescheduleMarkdownDoc = async (userId, cryptoKey, doc) => {
  const nextDate = getNextDate(doc.dueDate, doc.repeat);
  const payload = { ...doc, dueDate: nextDate };
  ...
  const encrypted = await encryptData(payload, cryptoKey);
  await updateDoc(doc(db, 'artifacts', appId, 'users', userId, 'markdown', doc.id), {
...
export const updateFolder = async (userId, cryptoKey, folderId, title) => {
  const encrypted = await encryptData({ title }, cryptoKey);
  await updateDoc(doc(db, 'artifacts', appId, 'users', userId, 'markdown', folderId), {
    ...encrypted, updatedAt: serverTimestamp()
  });
};
